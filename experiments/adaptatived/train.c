 ...
 * This training code is animated 
 */
void train(struct input_pattern * inputs[], int input_len) {
	...
	while (iteration < NUM_ITERATIONS) {
		...
		// UI changes - Keppler Alecrim 
		//Updating grid 
		for (k=0; k<GRIDS_XSIZE; k++) {
			for (j=0; j<GRIDS_YSIZE; j++) {
				color.red = (int)(grids[k][j]->weights[0] * 65535);
				color.blue = (int)(grids[k][j]->weights[1] * 65535);
				color.green = (int)(grids[k][j]->weights[2] * 65535);
				gtk_widget_modify_bg(drawingarea[k][j], GTK_STATE_NORMAL, &color);
			}
		}
 ...
 */
void train3(struct input_pattern * inputs[], int input_len) {
	int lw = GRIDS_XSIZE;
	int lh = GRIDS_YSIZE;
	int xstart, ystart, xend, yend;
	double dist, influence;
