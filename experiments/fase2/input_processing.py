...
'''
Esta classe lê os dados coletados pelo coletor e armazenados no arquivo
input.txt, transforma o primeiro elemento (MemFree) e uma subtracao da memoria
principal - MemFree. Desta forma, é possível calcular o consumo de memória x
tempo dos testes.
Esse cálculo já deveria ter sido feito no momento da coleta, porém, visto que
os testes são trabalhosos e deveriam ser refeitos, optei por fazer assim.
Uma outra coisa que esse programa faz é calcular o valor máximo e mínimo para o
consumo de memória, velocidade e aceleracão.
Este programa também gera um outro arquivo de log que deve ser passado à fase
4, que é de training da rede neural.
'''
