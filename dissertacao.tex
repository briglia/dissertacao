...
O algoritmo descrito foi implementado em um programa e testado na plataforma ARM. O código mostrado em \ref{alg:magic}, escrito em linguagem C, ilustra o algoritmo apresentado:
...
Para adaptar o CC ao consumo de memória, foi desenvolvido um \textit{daemon} que, dentre outras operações, utiliza o Algoritmo \ref{alg:magic} para determinar qual perfil executar do CC. Esse \textit{daemon} também é responsável por decidir que configuração realizar no CC.\\

Para facilitar os testes e limitar o universo de combinações das configurações do CC e do kernel do Linux, os perfis foram definidos em três \textit{scripts}. Um \textit{script} que desliga o CC, ou seja, retira a área de \textit{swap virtual} e libera todas as páginas de memória. Um \textit{script} que liga o CC e configura o mesmo com um tamanho de aproximadamente 10MB para o \textit{swap virtual} e um \textit{script} que liga o CC com um tamanho de aproximadamente 5MB, configura o \texttt{swappiness} para 60 e o \texttt{min\_free\_kbytes} para 2048. Resumindo e nomeando os scripts, temos:

\begin{itemize}
 \item Script 01: desligar o CC.
 \item Script 02: ligar o CC com tamanho igual a 10MB.
 \item Script 03: ligar o CC com tamanho igual a 5MB, swappiness = 60 e min\_free\_kbytes = 2048.
\end{itemize}

Os scripts são chamados de acordo com o fator definido.
