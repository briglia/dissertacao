...
O principal objetivo deste tipo de teste é visualizar como a memória é consumida ao longo do tempo e as situações de falta da mesma, quando várias aplicações são executadas pelo usuário. Para realizar os testes de consumo de memória, foi utilizado um cenário que representasse a interação do usuário com a máquina, com várias aplicações abertas, afim de impor uma grande quantidade de memória consumida.\\

Enquanto as aplicações eram executadas, um outro programa fazia leituras no \texttt{procfs}, coletando informações sobre a memória consumida pelas aplicações abertas e a memória usada pelo \textit{swap} virtual do Cache Comprimido, o ramzswap.

A Figura \ref{fig:behavior-test} mostra o consumo de memória através do tempo com o \texttt{max\_anon\_cc\_size} configurado para 1024 páginas (ou 4MB de tamanho).

\begin{figure}[htb]
 \centering
 \includegraphics[scale=0.5]{figs/behavior-no-cc.png}
 % behavior-no-cc.png: 640x480 pixel, 72dpi, 22.58x16.93 cm, bb=0 0 640 480
 \caption{Teste de consumo de memória usando Cache Comprimido.}
 \label{fig:behavior-test}
\end{figure}

Como pode-se notar na Figura \ref{fig:behavior-test}, uma característica do Cache Comprimido é utilizar uma parte da memória livre do sistema para alocar espaço para o swap virtual. Isso faz com que a memória livre disponível para o sistema seja menor do que se o Cache Comprimido não fosse utilizado. Em situações em que o consumo de memória é feito de forma muito rápida, o Cache Comprimido não consegue agregar benefícios ao sistema. Assim, o OOM (\textit{Out-of-memory killer}) é chamado e processos são finalizados para que alguma memória seja liberada.\\

Um pouco depois da metade da duração do teste, como visto na Figura \ref{fig:behavior-test}, quando o Cache Comprimido estava funcionando houve um aumento muito rápido no consumo de memória e no uso da área de swap virtual. Porém, a memória livre atingiu níveis muito baixos, o que ligou o mecanismo do OOM e algumas aplicações foram terminadas, e páginas comprimidas que estava armazenadas no swap virtual foram liberadas. Por essa razão nota-se um aumento abrupto na memória livre.\\

O teste de consumo de memória indica que o Cache Comprimido é útil quando o consumo de memória tem uma taxa constante ou que aumenta pouco no decorrer do tempo. Em situações em que uma quantidade alta de memória livre é requisitada de forma muito rápida, o Cache Comprimido não beneficia o sistema.
...
Nomeando os diferentes cenários de criação dos SOMs, temos as seguintes situações:
